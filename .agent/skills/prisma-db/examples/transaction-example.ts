...
async function processBookingWithWaitlist(
  userId: string,
  sessionId: string
) {
  return await prisma.$transaction(async (tx) => {
    ...
    const isAvailable = session.available && session.bookings.length < session.capacity
...
async function safeDecrementCredits(
  userId: string,
  amount: number
) {
