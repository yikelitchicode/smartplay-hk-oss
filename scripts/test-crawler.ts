...
async function main() {
	...
	const options = {
		component:
			(args.find((a) => !a.startsWith("-")) as "http" | "processor" | "all") ||
			"all",
		dryRun: args.includes("--dry-run"),
		liveApi: args.includes("--live"),
		verbose: args.includes("--verbose"),
		playDate: args.find((a) => a.startsWith("--play-date="))?.split("=")[1],
		faCode: args.find((a) => a.startsWith("--fa-code="))?.split("=")[1],
		distCode: args.find((a) => a.startsWith("--dist-code="))?.split("=")[1],
		lang: args.find((a) => a.startsWith("--lang="))?.split("=")[1],
		fixture:
			args.find((a) => a.startsWith("--fixture="))?.split("=")[1] ||
			"mock-response.json",
	};
	...
	const config = loadConfig({
		...(options.playDate ? { playDate: options.playDate } : {}),
		...(options.faCode ? { faCode: options.faCode.split(",") } : {}),
		...(options.distCode ? { distCode: options.distCode.split(",") } : {}),
	});
	...
	try {
		...
		if (options.component === "all" || options.component === "http") {
			...
			if (options.liveApi) {
				...
				logger.success(
					`All HTTP Requests successful (${duration.toFixed(1)}s)`,
				);
			...
			if (response && validation.valid) {
				...
				logger.table({
					...
					totalSessions: validation.summary.totalSessions,
					totalVenues: validation.summary.totalVenues,
				});
			} else {
		...
		if (options.component === "all" || options.component === "processor") {
			...
			if (typeof response !== "undefined") {
				logger.info(
					options.liveApi
						? "Using LIVE data from previous step..."
						: `Using fixture data from previous step (${options.fixture})...`,
				);
			...
			const processedData = processor.processRawResponse(
				rawResponse,
				"test-job-id",
				options.lang,
			);
