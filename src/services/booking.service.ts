...
import { withDbErrorHandling } from "@/lib/server-utils/error-handler";
import {
...
export async function getBookingDataService(
	...
	return withDbErrorHandling(async () => {
			...
			if (venue) {
				...
				if (!venue.facilities[code]) {
					venue.facilities[code] = {
						...
						nameEn: session.facilityType?.nameEn || session.facilityTypeNameEn,
		...
		const availableDistricts = Array.from(districtMap.values()).sort((a, b) =>
