										...
										{facility.sessions.map((session) => {
											...
											return (
												<button
													...
													aria-label={`${session.startTime} - ${session.available ? "Available" : "Booked"}${session.isPassed ? " (Passed)" : ""}`}
													className={`relative group flex flex-col items-center justify-center p-2 rounded-lg text-sm font-medium border transition-all ${getSessionStyles()}`}
												>
